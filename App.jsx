...
const STORAGE_KEY = "ingles_dinamita_progress_v1";
...
function Topbar({ route, setRoute }) {
  return (
    <div className="sticky top-0 z-20 backdrop-blur bg-white/80 border-b border-neutral-200">
      <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-xl bg-neutral-900 text-white grid place-items-center font-bold">EN</div>
          <div className="font-semibold">Inglés Dinamita</div>
...
function Home({ setRoute, progress }) {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-neutral-600 mb-6">Curso por niveles con micro-lecciones, práctica y chat de dudas. Empieza por tu nivel.</p>
      <div className="mb-6">
          ...
          <div className="space-y-2">
            {lesson.dialog.map((d, idx) => (
              <div key={idx} className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div className="p-3 rounded-xl bg-neutral-100 text-sm">{d.en}</div>
                <div className="p-3 rounded-xl bg-neutral-50 text-sm text-neutral-700">{d.es}</div>
              </div>
            ))}
          </div>
          <div className="mt-3 text-xs text-neutral-500">Tip: mantén presionado para usar TTS del teléfono.</div>
        </section>
      ) : null}
        ...
        <section className="mb-6 rounded-2xl border border-neutral-200 p-4">
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {lesson.vocab.map((v, i) => (
              <div key={i} className="p-3 rounded-xl bg-neutral-50 text-sm flex items-center justify-between">
...
function ChatDudas() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <p className="mt-2 text-xs text-neutral-500">Nota: En la versión completa, este chat usa IA (API) y acepta audio/foto.</p>
    </div>
  );
}
    ...
    <div className="min-h-dvh bg-white text-neutral-900">
      ...
      <footer className="mt-10 border-t border-neutral-200">
        <div className="max-w-5xl mx-auto px-4 py-6 text-xs text-neutral-500 flex items-center justify-between">
          <span>© {new Date().getFullYear()} Inglés Dinamita</span>
          <span>Diseñado por Dinamita Apps</span>
        </div>
      </footer>
    </div>
  );
